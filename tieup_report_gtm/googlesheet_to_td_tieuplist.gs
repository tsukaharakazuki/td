...
function getData_tieuplisttemp() {
  ...
  // 2行目A列~最終行M列のデータを取得
  var select = as.getRange(2,1,lastRow - 1,13).getValues();
  var records = [];
  for (var i = 0; i < (lastRow-1);i++){
    ...
    records.push({
      ...
      "c_4": select[i][8],
      "c_5": select[i][9],
      "ls_page_url": select[i][10],
      "ls_client_name_jp": select[i][11],
      "ls_page_title": select[i][12],
