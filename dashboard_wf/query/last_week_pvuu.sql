...
FROM 
  ...
  WHERE
    ...
    TD_TIME_RANGE
      (time, 
        td_time_format(TD_SCHEDULED_TIME() - (cast(td_time_format(TD_SCHEDULED_TIME(), 'u', 'JST') as integer) + 7) * 3600 * 24 , 'yyyy-MM-dd', 'JST'), 
        td_time_format(TD_SCHEDULED_TIME() - cast(td_time_format(TD_SCHEDULED_TIME(), 'u', 'JST') as integer) * 3600 * 24, 'yyyy-MM-dd', 'JST'),
      'JST')
  )
GROUP BY 1,2
