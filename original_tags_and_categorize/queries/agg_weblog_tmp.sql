...
(
  ...
  FROM
    ${log_db}.${log_tb}
  WHERE
    TD_INTERVAL(time, '-90d/now', 'JST') AND
...
--  event_type ,
  ...
  td_ref_host ,
  CASE
    WHEN td_ref_host = 'plat.navitime.co.jp' THEN 'ナビタイム'
    WHEN td_ref_host = 'social-admin.userlocal.jp' THEN 'UserLocal'
